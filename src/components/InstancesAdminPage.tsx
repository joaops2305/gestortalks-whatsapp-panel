...
import { Alert, Box, Button, Card, Divider, InputAdornment, Stack, TextField, Typography } from '@mui/material';
...
export function InstancesAdminPage() {
  ...
  async function action(instance: WhatsAppInstance, type: 'disconnect' | 'logout') {
    setBusyId(instance.id); setError('');
    try {
  ...
  async function remove(instance: WhatsAppInstance) {
    ...
    try { await instanceService.remove(instance.id); removeLocal(instance.id); }
    catch (requestError: any) { setError(requestError?.response?.data?.error || 'Não foi possível excluir a instância.'); }
    finally { setBusyId(null); }
  }
  ...
  return <AuthGuard><AppShell><Stack spacing={3}>
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" spacing={2} alignItems={{ sm: 'center' }}>
      <Box><Typography variant="h4" fontWeight={800}>Instâncias</Typography><Typography color="text.secondary">Gerencie sessões, conexão, QR Code e status dos números.</Typography></Box>
    ...
    <Card variant="outlined" sx={{ overflow: 'hidden' }}>
      ...
      <Divider />
      <InstanceListTable rows={paginated} loading={loading} busyId={busyId} onQr={setSelected} onDisconnect={(instance) => void action(instance, 'disconnect')} onLogout={(instance) => void action(instance, 'logout')} onDelete={(instance) => void remove(instance)} />
  ...
  <InstanceFormDialog open={formOpen} onClose={() => setFormOpen(false)} onCreated={(instance) => { insertLocal(instance); setPage(1); }} />
  <InstanceQrDialog instance={selected} open={Boolean(selected)} onClose={() => setSelected(null)} onChange={(changes) => selected && patch(selected.id, changes)} />
  </AppShell></AuthGuard>;
}
